...
public class Run {
	...
	public static void main(String[] args) {
		 ...
		 * 
		 * MAC(Media Access Controller) Adderss / IP(Internet Protocol) Addresss
		 * 
		 * - MAC Addresss : NIC(Network Interface Card)에 실제 제조시 붙여지는 물리적인 주소
		 * 
		 ...
		 *   		  즉, 서비스를 제공받는 고객
		 *   
		 *   => 서버는 자기 혼자 응답을 보낼 수 없음, 클라이언트의 요청이 들어와야만 응답을 보낼 수 있는구조
		 *   => 클라이언트는 서버에 요청을 보내기 위해서 서버의 IP주소와 Port 번호르 알아야함
		 */
		
		// InetAddresss : 네트워크 정보를 확인할 수 있는 클래스
		try {
			InetAddress localHost = InetAddress.getLocalHost();
			// localHost : 현재 프로그램을 동작시킨 장치를 의미
			// getLocalHost(): 작업하고 있는 Host의 정보를 반환해주는 메서드
			...
			// 도메인 네임을 통해서 서버컴퓨터와 관련된 정보를 얻어 낼 수 있음
			...
			// getByName() : 도메인 네임을 이용해 서버의 정보를 반환
