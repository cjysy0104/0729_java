...
public class Run {
	...
	public static void main(String[] args) {
		 ...
		 * - 배열의 불편한 점
		 * 
		 * 1. 배열을 사용하려고 할 때 크기를 꼭 할당을 해야함. => 한 번 지정된 크기는 변경이 불가능
		 * 2. 배열 중간 위치에 새로운 요소를 추가하거나 삭제할 때 기존값을 밀어주거나 당기는 코드를 써야 함.
		 * 
		 * - 컬렉션
		 * 1. 크기의 제약이 없음
		 * => 처음 컬렉션을 사용할 때 크길르 지정하지 않더라도 사용할 수 있음
		 * => 만약 요소가 추가되면서 크기를 넘어서는 상황이 발생하면 알아서 확장됨.
		 * 2. 중간에 값을 추가하거나 삭제하는 경우 기존값을 밀어주가나 땡겨주는 로직이 이미 구현되어있음
		 * => 그때그때 필욯나 메서드들을 호출해서 사용하면 됨
		 * 
		 * --중립
		 ...
		 * 				-> ArrayList
		 *
		 * - Set 계열 : 다루고자 하느 값(vlaue)만 저장ㅇ, 저장시 순서유지 X, 중복 허용 X 
		 * 				=> HashSet, TreeSet	
		 * 				-> HashSet
		 * 
		 * - Map 계열 : 키(Key) - 값(Value) 세트로 저장 / 저장시 순서 유지X, 키 중복 X, 값중복 O
		...
		// 1. 기본 생성자로 호출 만들기 => 기본크기 10칸
		// ArrayLIist 식별자 = new ArrayList(정수);
		// 2. 인자로 전달한 정수값만큼의 크기를 가진 리스트를 만들겠다.
		ArrayList plants = new ArrayList(3);
		// 내부적으로 크기가 3칸인 List를 만들겠다.
		// 배열타입[] arr= new 배열타입[3]
		 ...
		 */
		// 1. 비어있는 List에 요소를 추가
		// add (E e) : 해당 리스트 마지막에 인자로 전달된 요소를 추가해주는 메서드
		// E -> element : 제너릭
		...
		// Arrays.copyOf메서드를 이용하여 깊은 복사로 더 큰 배열로 할당함
		// Object타입으로 요소를 관리하기 때문에 여러 자료형의 요소를 하나의 리스트로 담을 수 있음
		
		// 1. 3칸을 만들었지만 추가 O
		// 2. 다른 자료형이 들어감 ( 둘 다 참조자료형Object로 형변환 가능)
		System.out.println(plants.toString());
		
		// add메서드 호출 시 오버로딩된 형태의 메서드를 사용
		// add(int index, E e) : List의 index에 전달한 요소를 추가
		plants.add(0, "식물 시작!");
		plants.add(3, new Plant("관엽", "홍콩야자"));
		
		System.out.println(plants);
		// 중간위치에 요소를 추가했을 때 기존의 요소들을 한 칸씩 미렁주는 작업이 내부적으로 구현되어있음
		
		// 2. 요소를 수정하는 메서드
		// set(int index, E e) : List의 index위치에 값을 전달된 요소(e)로 set해주는 메서드
		plants.set(0, "시~작~");
		
		System.out.println(plants);
		
		// 3. 요소 삭제
		// remove(int index) : List의 index위치에 담긴 요소를 삭제하는 메서드
		plants.remove(0);

		//plants.remove(5); // 0번을 제거했으므로 4번까지밖에 없음: IndexOutOfBound~
		

		String str = (String)plants.remove(4); // 다운캐스팅
		...
		// 5. List의 해당 index 위치에 담긴 요소를 반환받는 메서드
		// get(int index) : E
		...
		// 7. 리스트에 요소를 왕창 추가하기
		// addAll(Collection c) : 해당 리스트에 다른 컬렉션에 있는 요소들을 몽땅 추가해주는 메서드
//		ArrayList l;
//		LinkedList i;
//		Vector v;
//		HashSet h;
//		TreeSet t;
		// ~List : List 인터페이스, ~Set : Set인터페이스 >> 모두 Collection이 부모이기때문에 사용법 같고 upcasting가능
		plants.addAll(sub);
		System.out.println(plants); // 리스트는 데이터 값(Value)이 중복될 수 있음
		
		// 8. 리스트에 현재 요소가 있는지 없는지 판별하는 메서드
		// isEmpty(): 리스트에 요소가 존재하지 않다면 true / 하나라도 있다면 false
