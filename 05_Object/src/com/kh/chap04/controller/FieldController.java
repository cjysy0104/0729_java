 ...
 * - (local) variable : 클래스 영역 내부에 특정한 구역에서 선언됨( { } )	** 자바에선 전역변수는 존재X
 * 						=> method, for, if 등등..
 * 
 * 1. 전역 변수의 역할을 하는 친구들 (전역변수가 아님, 자바엔 전역변수 존재 X)
 * 
 * - 필드(멤버변수, 인스턴스 변수) : 
 * 							생성 시점 => new 키워드 사용해서 해당 객체를 생성 하는 순간 heap에 할당
 * 							소멸 시점 => 객체가 소멸될 때(G.C가 heap 인스턴스 지울 때)
...
public class FieldController {
	...
	public void checkVariable(int parameter) {
		...
		// local(지역변수) 할당 및 초기화
		int local = 1; // 지역변수 특징: 초기화 안하면 사용할 수 없다.
